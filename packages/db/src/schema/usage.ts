...
// The chat agent runtime (assistant messages) writes here, so the dashboard
// can aggregate across models/sessions without parsing JSONB shapes.
//
// `dedup_key` is unique: rows use `chat:<messageId>` — an upsert on conflict
// keeps re-finalizes / replays from double-counting. `priced` is false when no
// pricing was known (so the UI can show "unknown" instead of a silent $0). See
...
export const usageRecords = pgTable(
	"usage_records",
	{
		...
		// USD (chat's cents are divided by 100 on write). null when `priced` is
		// false.
