...
export function collectOpenRequestRows(
	...
	for (const { seq, event } of rows) {
		...
		// see apps/bridge-cli/src/adapters/approvals.ts / questions.ts) closes
		// the id exactly like a retraction: the answer COMMAND may be long gone
		// from the relay's TTL'd window, but the resolution event survives in
