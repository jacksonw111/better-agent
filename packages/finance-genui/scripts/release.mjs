#!/usr/bin/env node
// One-click release for @jacksonw111/finance-genui.
//
//   pnpm -F @jacksonw111/finance-genui release <patch|minor|major|x.y.z>
//
// Pre-flights (typecheck + test + build), bumps the version, commits, tags
// `finance-genui-v<version>`, and pushes — the tag triggers the GitHub Packages
...
if (!bump) {
	fail(
		"Usage: pnpm -F @jacksonw111/finance-genui release <patch|minor|major|x.y.z>"
	);
}
...
const tag = `finance-genui-v${version}`;

shIO(`git add "${pkgJsonPath}"`);
shIO(`git commit -m "chore(finance-genui): release v${version}"`);
shIO(`git tag ${tag}`);
